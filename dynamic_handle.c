...
//returns true is v is in the list of nbrs, false o.w.
bool in_nbrs(int* nbrs, int size, int v){
...
static void * fault_handler_thread(void *arg){
	...
	*/
	...
	while( !needQuit(mx) ) {
		...
		int* curr_nbrs = get_nbrs(G,faulting_node);
		int  curr_deg  = get_deg(G,faulting_node);
		int  curr_off  = get_off(G,faulting_node);

		int num_nodes_per_page = page_size/( sizeof(int)*G->D );

		//th smallest offest on this page
		int lower_page_off = (curr_off/num_nodes_per_page)*num_nodes_per_page;
		int upper_page_off = lower_page_off + num_nodes_per_page -1;

		int nbr_off;

		printf("faulting node: %d, offset: %d\n", faulting_node, curr_off);

		int off_count = lower_page_off;
		int num_swapped = 0;
		for(int i=0; i<curr_deg; i++){
			if(curr_nbrs[i] == 0){
				perror("zero node in handler");
				exit(EXIT_FAILURE);
			}

			//Don't swap more nodes than can fit on a page
			if(off_count > upper_page_off){
				//printf("off > ub\n");
				break;
			}
			nbr_off = get_off(G,curr_nbrs[i]);
			//already on the page
			//printf("node: %d, off: %d\n", curr_nbrs[i], nbr_off);

			if( lower_page_off <= nbr_off && nbr_off <= upper_page_off  ){
				//printf("already in range\n");
				continue;
			}

			int to_swap = get_node_from_off(G,off_count);
		
			if( off_count == curr_off || in_nbrs(curr_nbrs, curr_deg, to_swap )){
				//printf("in_nbrs or curr_node\n");
				off_count++;
				continue;
			}
			
			printf("swapping %d with %d\n", to_swap, curr_nbrs[i]);
			//print_node(G,to_swap);
			//print_node(G,curr_nbrs[i]);

			//printf("at %d and %d\n", off_count, nbr_off);

			swap_nodes(G,to_swap ,curr_nbrs[i]);
			//printf("seetin app_G node %d to off %d\n",curr_nbrs[i],off_count);
			set_off(app_G, curr_nbrs[i], off_count);
			//printf("settin app_G node %d to off %d\n",to_swap,nbr_off);
			set_off(app_G, to_swap, nbr_off);
		...
		//TODO Don't forget to also fix the other graphs offsets

		/****************************************************************/

		int page_num = ((b_addr - ((ul) start_addr))/page_size) ;

		//Copy the page of the graph in 
